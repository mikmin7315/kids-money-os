        ...
        <section className="mt-6">
          <Surface className="bg-[linear-gradient(135deg,rgba(255,248,236,0.98),rgba(232,244,240,0.92))]">
            ...
            <p className="mt-2 text-sm leading-6 text-[var(--color-muted)]">
              마지막 관리자 계정은 DB 함수가 자동으로 강등을 막습니다.
            </p>
          </Surface>
        </section>
  ...
  return (
    <Surface className="bg-[linear-gradient(180deg,rgba(255,255,255,0.72),rgba(249,243,234,0.95))]">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="font-display text-xl font-semibold">{profile.name || "이름 없음"}</p>
          <p className="mt-1 text-sm text-[var(--color-muted)]">{profile.email || "이메일 없음"}</p>
        </div>
        <Badge tone={profile.role === "admin" ? "rose" : "sky"}>{profile.role}</Badge>
      </div>
