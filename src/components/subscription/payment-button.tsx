"use client";

import { useRouter } from "next/navigation";
import { useState } from "react";

export function PaymentButton({
  userId,
  userEmail,
  userName,
}: {
  userId: string;
  userEmail: string;
  userName?: string;
}) {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  async function handlePayment() {
    setLoading(true);
    setError(null);
    try {
      const { requestIssueBillingKey } = await import("@portone/browser-sdk/v2");
      const issueId = `bk-${userId.slice(0, 8)}-${Date.now().toString(36)}`;

      const result = await requestIssueBillingKey({
        storeId: process.env.NEXT_PUBLIC_PORTONE_STORE_ID ?? "",
        channelKey: process.env.NEXT_PUBLIC_PORTONE_CHANNEL_KEY ?? "",
        billingKeyMethod: "CARD",
        issueId,
        issueName: "모나리 플러스 구독",
        customer: {
          customerId: userId,
          email: userEmail,
          ...(userName ? { fullName: userName } : {}),
        },
      });

      if (!result || result.code) {
        setError(result?.message ?? "카드 등록에 실패했어요.");
        return;
      }

      const res = await fetch("/api/payment/billing-key", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ billingKey: result.billingKey }),
      });

      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        setError((data as { message?: string }).message ?? "결제 처리 중 오류가 발생했어요.");
        return;
      }

      router.refresh();
    } catch (e) {
      setError(e instanceof Error ? e.message : "결제 중 오류가 발생했어요.");
    } finally {
      setLoading(false);
    }
  }

  return (
    <div>
      <button
        onClick={handlePayment}
        disabled={loading}
        className="w-full rounded-[16px] bg-white py-3.5 text-[15px] font-black text-[#7C3AED] transition active:scale-[0.97] disabled:opacity-60"
      >
        {loading ? "카드 등록 중..." : "월 3,900원으로 시작하기"}
      </button>
      {error && (
        <p className="mt-3 text-center text-[13px] font-semibold text-red-300">{error}</p>
      )}
    </div>
  );
}
